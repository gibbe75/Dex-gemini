...
> **Gemini CLI is the recommended starting point** — it's free, open-source, and has a generous daily quota. Install with `npm install -g @google/gemini-cli`, then run `gemini` in your vault folder.

**Not sure which?** Start with Gemini CLI — it's free and gets you running in minutes.
...
If you're using Gemini CLI, here's the fastest path:
...
# 3. Run the installer (sets up Python env + Gemini CLI MCP config)
bash install.sh
