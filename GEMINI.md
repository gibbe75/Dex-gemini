...
**Gemini CLI setup:** MCP servers are configured in `~/.gemini/settings.json`. The installer
generates this from `System/.gemini.settings.json.example`. Skills live in `.claude/skills/`
(same folder as always — Gemini CLI reads them from disk via skill instructions in this file).
============================================================ -->

**Last Updated:** May 21, 2026 (Gemini CLI support)
...
**Automation:** When meetings are processed via `/process-meetings`, run the post-meeting person-update script manually if needed:
```bash
node .claude/hooks/post-meeting-person-update.cjs <meeting-file-path>
```
...
After making significant system changes (new commands, GEMINI.md edits, structural changes), update `CHANGELOG.md` before finishing the task.
...
### Context Enrichment (Manual Hooks)
Context enrichment scripts are available in `.claude/hooks/` and can be run on demand:
...
```bash
node .claude/hooks/person-context-injector.cjs <file-path>
node .claude/hooks/company-context-injector.cjs <file-path>
```
...
- **Never ask again**: 
  ...
  - Remove this section from GEMINI.md
...
Skills extend Dex capabilities and are invoked with `/skill-name`. To load a skill, read its SKILL.md file from disk. Common skills include:
...
- `/dex-update` - Update Dex automatically (shows what's new, updates if confirmed, also updates GEMINI.md)
